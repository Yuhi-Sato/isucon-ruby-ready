...
remote-deploy-%: ## ローカルから対象サーバーにデプロイする（remote-deploy-s1 など）
	ssh $* "cd $(REMOTE_DEPLOY_PATH) && ./deploy.sh"

.PHONY: remote-deploy-all
remote-deploy-all: ## ローカルから全サーバーに順次デプロイする（対象は SERVERS で調整）
	@failed=""; \
	for s in $(SERVERS); do \
		$(MAKE) remote-deploy-$$s || failed="$$failed $$s"; \
	done; \
	if [ -n "$$failed" ]; then echo "deploy failed on:$$failed"; exit 1; fi
