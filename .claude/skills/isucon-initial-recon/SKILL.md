...
isucon-user-behavior-analysis スキルの導入手順に従い、Rackミドルウェア（`X-User-Id`ヘッダー付与）と nginxの`proxy_hide_header`を入れる。手順3で把握したセッションのユーザーIDキーを使う。
ここで入れておくと、ベースラインを含む**以降すべてのベンチで行動履歴（userid付きアクセスログ）が自動的に手に入る**。後から入れると、欲しくなった瞬間のベンチデータにはuseridが無い。
