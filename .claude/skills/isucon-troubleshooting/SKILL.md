...
3. `make journal-errors` でアプリ側のエラーログ（5xx・例外スタックトレース）を抽出する。全量を追いたいときは `make watch-service-log`
...
| サーバーのログを見ずに推測でコードを直す | `make journal-errors` / `make watch-service-log` で実際のエラーを確認してから直す |
