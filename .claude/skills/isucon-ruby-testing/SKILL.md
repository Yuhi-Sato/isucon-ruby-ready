...
description: ISUCONのRuby実装で、リファクタ前にハンドラをDocker + minitest/rack-testで回帰テストするスキル。フレームワーク（Sinatra等）・出題ドメイン非依存のテスト環境構築・test_helper・実行手順・ハマり対策をまとめる。「テストを書いて」「リファクタ前に動作確認したい」「このハンドラを検証して」などのリクエストで使用する。**本スキルの担当はテストの作成・実行のみ**で、N+1解消・インデックス追加・リファクタ・計測・デプロイ等は一切行わない。
---
...
**本スキルの作業は「テストの作成・実行」のみ。** それ以外（サーバー操作・計測・デプロイ・アプリのリファクタや N+1/インデックス修正・設定チューニング）は一切行わない。該当作業は対応する他スキルに委譲し、本スキルではテストを作ることに徹する。

> **N+1解消・インデックス追加・リファクタ手順は本スキルでは扱わない。**

## 適用条件

以下のいずれかのリクエストでのみ使う（テスト作成に関わる場合に限定）:

- 「リファクタ前にハンドラを Docker+minitest で検証する」
- 「テストを書いて」
- 「このエンドポイントをテストで固定したい」

テスト作成と無関係な依頼（計測・デプロイ・リファクタ等）には本スキルを適用しない。
