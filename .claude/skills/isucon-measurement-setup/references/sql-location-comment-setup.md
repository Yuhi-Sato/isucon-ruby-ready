...
3. commit・push・デプロイ後、ベンチを1回回してから `make slow-query` の詳細ブロック（`\G`縦形式）の `query_sample` 列（`QUERY_SAMPLE_TEXT`）に `/* app.rb:123 */` のようなコメントが付いているか確認する

   `query`列（`DIGEST_TEXT`）は正規化済みでSQLコメントが常に除去されるため、ここには位置コメントは絶対に出てこない。位置コメントは`query_sample`列でのみ確認できる。
...
| `query`列（`DIGEST_TEXT`）でコメントを探して「付いていない」と勘違いする | `DIGEST_TEXT`はダイジェスト計算時にコメントが常に除去される仕様。`query_sample`列（`QUERY_SAMPLE_TEXT`）を見る |
| 同じSQLパターンを複数箇所から発行している場合、`query_sample`は最も遅かった1回分のfile:lineしか示さない | 他の発行元も知りたい場合はアプリを`grep`するか、一時的にアプリ側のコメントを見分けやすい形にして絞り込む |
