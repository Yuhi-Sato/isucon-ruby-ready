...
make remote-bench-prep-all  # 全サーバーで順に実行（SERVERS="s1 s2" で絞れる）

# ブランチを指定する場合
make remote-bench-prep-all BRANCH=feature/example
```

`make remote-bench-prep-all` は DB を含む全再起動を伴うため、並列ではなく `SERVERS` の順（s1 → s2 → s3）に直列で実行する。
