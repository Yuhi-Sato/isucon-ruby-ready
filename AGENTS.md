...
競技のフェーズに対応するスキルを用意している。該当フェーズの作業を始める前に必ず対応スキルを読む。

| スキル | 使うタイミング |
|---|---|
| `isucon-initial-recon` | 競技開始直後の初動調査（レギュレーション確認〜ベースライン記録） |
| `isucon-bottleneck-analysis` | ベンチ後、計測結果から次の改善対象を決めるとき |
| `isucon-optimization-patterns` | アプリコードの改善（N+1・インデックス・キャッシュ等）を実装するとき |
| `isucon-server-tuning` | MySQL/nginx/アプリサーバー設定の変更や複数台構成への分割 |
| `isucon-final-check` | 終了約1時間前からの最終確認（ログ無効化・再起動試験・最終ベンチ） |
