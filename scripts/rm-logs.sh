...
# アクセスログ・エラーログ・スロークエリログ・クエリダイジェスト統計を空にする（make rm-logs の本体）。
...
sudo test ! -f "$NGINX_ERROR_LOG" || sudo truncate -s 0 "$NGINX_ERROR_LOG"

# /var/log/mysql はisuconユーザーから読めない(750 mysql:adm)ため、存在確認もsudoで行う
sudo test ! -f "$DB_SLOW_LOG" || sudo truncate -s 0 "$DB_SLOW_LOG"
sudo test ! -f "$DB_ERROR_LOG" || sudo truncate -s 0 "$DB_ERROR_LOG"
